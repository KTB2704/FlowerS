...
public class OrderAdminActivity extends AppCompatActivity {
    private TextView  btnAccept, btnHistory;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseRef = FirebaseDatabase.getInstance().getReference("orders/admin/pending");

        switchActivity(); // Chuyển đổi intent giữa các trạng thái đơn hàng

        loadPendingOrdersAdmin(); // Load đơn hàng đang chờ từ firebase
    ...
    private void loadPendingOrdersAdmin() {
    ...
    private void switchActivity() {
        btnAccept.setOnClickListener(v -> {
            Intent intent = new Intent(OrderAdminActivity.this, OrderAdminAcceptActivity.class);
            overridePendingTransition(0, 0);
            startActivity(intent);
        });

        btnHistory.setOnClickListener(v -> {
            Intent intent = new Intent(OrderAdminActivity.this, OrderAdminHistoryActivity.class);
            overridePendingTransition(0, 0);
            startActivity(intent);
        });
    }
