...
public class DeliveryUserActivity extends AppCompatActivity {
    private TextView btnPending, btnAccept, btnCancel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Truy cập node orders/users/" + userId + "/delivery của người dùng trong Firebase
        databaseReference = FirebaseDatabase.getInstance().getReference("orders/users/" + userId + "/delivery");

        switchActivity(); // Chuyển đổi intent giữa các trạng thái đơn hàng

        loadDeliveryOrders(); // Load đơn hàng đang giao từ firebase
    ...
    private void loadDeliveryOrders() {
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Log.e("DeliveryUserActivity", "Lỗi tải đơn hàng: " + error.getMessage());
            }
        });
    }
    ...
    private void switchActivity() {
        btnPending.setOnClickListener(v -> {
            Intent intent = new Intent(DeliveryUserActivity.this, PendingUserActivity.class);
            overridePendingTransition(0, 0);
            startActivity(intent);
        });

        btnAccept.setOnClickListener(v -> {
            Intent intent = new Intent(DeliveryUserActivity.this, SuccessUserActivity.class);
            overridePendingTransition(0, 0);
            startActivity(intent);
        });
        btnCancel.setOnClickListener(v -> {
            Intent intent = new Intent(DeliveryUserActivity.this,CancelUserActivity.class);
            overridePendingTransition(0, 0);
            startActivity(intent);
        });
    }
